...
source("~/Desktop/mouse_barcoding_last/srcvisualization/0_config/0_config.R")
library(tidyr)
library(umap)
library(ggforce)
library(factoextra )
...
get_time_derivative_gavaged = function(taxa,sample){###colonized cohorts with microbiota
...
get_time_derivative_clone=function(sample){###linegae time derivatives
  clusters.loess=read_csv(file= paste("reports//figures/clustering/",sample,"/",sample,"_clustered_loess_log10.csv",sep=""))
...
plot_eigen_value_evolution=function(all_eigen_value,sample){
  ...
  for(i in seq_along(eigen_space)){
    ...
    ggsave(plots_eigen_value[[i]],
           filename = paste("reports//figures/DCM/eigen_values/",sample, "_eigen_value_",eigen_space[i],"_time_interval.eps", sep=""),width =10.5,height =7.5, limitsize = FALSE,device = cairo_ps )
  }
...
get_chosen_umap=function(df,n,method,sample){
  ...
  saveRDS(final,paste0("reports/figures/DCM/Umap",sample,"_",n,"_",method))
...
plot_umap_cluster_quantification=function(sample){
  tf=read_csv(file =paste0("reports_article/figures/DCM/UMAP_csv/",sample,"_cluster_size_umap.csv"))
  ...
  ggsave(p2,filename =paste0("reports/figures/DCM/umap_cluster_quantify_",sample,".eps",sep=""), width =8.25,height =6,limitsize = FALSE )       
...
plot_derivative=function(series_normal_inter,deriv_series_loess,sample){
  ...
  ggsave(p,filename = paste0("reports_article//figures/DCM/fitness/", sample, "_series_fitting.eps"),height = 6,width = 45)
}
